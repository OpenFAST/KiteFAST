...
!====================================================================================================
subroutine VSM_WriteCoords( u, p, errStat, errMsg )
! This subroutine writes the data stored in WriteOutputs (and indexed in OutParam) to the file
! opened in VSM_Init()
!---------------------------------------------------------------------------------------------------- 

      ! Passed variables   
   type(VSM_InputType),          intent( in    ) :: u                    ! VSM module's output data
   type(VSM_ParameterType),      intent( in    ) :: p                    ! VSM module's parameter data
   integer,                      intent(   out ) :: errStat              ! returns a non-zero value when an error occurs  
   character(*),                 intent(   out ) :: errMsg               ! Error message if ErrStat /= ErrID_None
   ...
   OutFileName = TRIM(p%OutRootName)//'.VSM.pts'
   call GetNewUnit( Un )
   ...
   write (Un,'(/,A/)', IOSTAT=errStat)  'These coordinates were generated by '//TRIM(VSM_Ver%Name)//&
                      ' on '//CurDate()//' at '//CurTime()//'.'
   
         ! Write the names of the output parameters:
   
   write(Un,'(A9,6(1X,A11))')  'Element #', 'PointAX', 'PointAY', 'PointAZ', 'PointBX', 'PointBY', 'PointBZ'
      
   do i = 1, p%NumElem
      write(Un, '(I9,6(1X,F11.4))') i, u%PtA(1,i), u%PtA(2,i), u%PtA(3,i), u%PtB(1,i), u%PtB(2,i), u%PtB(3,i)   
   end do
   
   close(Un)   
      
end subroutine VSM_WriteCoords
      ...
      do i = 1, numElem
         ...
         ! Calculate U_AB
            ...
            if ( tmp1c0  > r0*e2 ) then
               ...
               if ( EqualRealNos(FactorAB,0.0_ReKi) ) then
...
!..............................................................................
   ...
   call VSM_WriteCoords( u, p, errStat, errMsg )
      if ( errStat >= AbortErrLev ) return
