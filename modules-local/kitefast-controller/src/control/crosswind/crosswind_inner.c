...
static void ScheduleLateralGains(
    double airspeed, double alternate_gain_fade,
    const CrosswindInnerParams *params,
    double lateral_gains[][kNumCrosswindLateralStates]) {
  assert(airspeed >= 0.0);
  assert(alternate_gain_fade >= 0.0 && alternate_gain_fade <= 1.0);
  assert(params != NULL && lateral_gains != NULL);
  typedef double LateralGainArray[kNumCrosswindLateralInputs]
                                 [kNumCrosswindLateralStates];
  LateralGainArray lateral_gains_nom;
  LateralGainArray lateral_gains_alt;
  ...
  if (airspeed < params->airspeed_table[kCrosswindInnerNominalAirspeed]) {
    CrossfadeArray(&params->lateral_gains_min_airspeed[0][0],
                   ...
                   airspeed, params->airspeed_table[kCrosswindInnerMinAirspeed],
                   params->airspeed_table[kCrosswindInnerNominalAirspeed],
                   &lateral_gains_nom[0][0]);
    CrossfadeArray(&params->lateral_gains_alt_min_airspeed[0][0],
                   &params->lateral_gains_alt_nominal_airspeed[0][0],
                   kNumCrosswindLateralInputs * kNumCrosswindLateralStates,
                   airspeed, params->airspeed_table[kCrosswindInnerMinAirspeed],
                   params->airspeed_table[kCrosswindInnerNominalAirspeed],
                   &lateral_gains_alt[0][0]);
  } else {
    CrossfadeArray(&params->lateral_gains_nominal_airspeed[0][0],
                   ...
                   params->airspeed_table[kCrosswindInnerMaxAirspeed],
                   &lateral_gains_nom[0][0]);
    CrossfadeArray(&params->lateral_gains_alt_nominal_airspeed[0][0],
                   &params->lateral_gains_alt_max_airspeed[0][0],
                   kNumCrosswindLateralInputs * kNumCrosswindLateralStates,
                   airspeed,
                   params->airspeed_table[kCrosswindInnerNominalAirspeed],
                   params->airspeed_table[kCrosswindInnerMaxAirspeed],
  ...
  CrossfadeArray(&lateral_gains_nom[0][0], &lateral_gains_alt[0][0],
                 kNumCrosswindLateralInputs * kNumCrosswindLateralStates,
                 alternate_gain_fade, 0.0, 1.0, &lateral_gains[0][0]);
}
...
bool CrosswindInnerValidateParams(const CrosswindInnerParams *params) {
  if (!(params->airspeed_table[kCrosswindInnerMinAirspeed] > 0.0)) {
    assert(!"The first element of airspeed_table must be positive.");
    return false;
  }

  for (int32_t i = 1; i < ARRAYSIZE(params->airspeed_table); ++i) {
    if (params->airspeed_table[i - 1] >= params->airspeed_table[i]) {
      assert(!"airspeed_table must be strictly monotonically increasing.");
      return false;
    }
  }
...
//
// TODO(kennyjensen): The tether roll and beta integrators can result
...
static void ControlLateral(double tether_roll_cmd, double tether_roll,
                           ...
                           double *alt_lateral_gains_0) {
  assert(params->alt_gain_fade_rate > 0.0);
...
static double ControlAirspeed(double airspeed_cmd, double airspeed,
  ...
  // TODO(kennyjensen): The M600 does not pass the IEC cases when the
  ...
  // root cause.
  *delta_spoiler =
      flags->spoiler_enabled && state->spoiler_on ? params->delta_spoiler : 0.0;
  // Rate limit the spoiler as suggested by nathantreat@ to minimize the effect
  // of unsteady transients.
  *delta_spoiler = RateLimit(*delta_spoiler, params->delta_spoiler_on_rate,
                             params->delta_spoiler_off_rate, *g_sys.ts,
                             &state->delta_spoiler_z1);
...
void CrosswindInnerStep(double tether_roll_cmd, double tether_roll,
  ...
  ControlLateral(tether_roll_cmd, tether_roll, beta_cmd, beta, pqr_cmd, pqr,
                 airspeed, loop_angle, state->loop_angle_z1, flags, exp_config,
                 params, state, lateral_gains, deltas, &thrust_moment->moment.z,
                 &state->alt_lateral_gains_0);
