...
double GetPlaybookEntryAzimuthWithLimits(double wind_dir,
  ...
  for (int32_t i = 0; i <= num_steps; ++i) {
    ...
    // The edges of the loop result in azimuth offsets different than the half
    // angle due to the effect of elevation.
    // See here for derivation: https://screenshot.googleplex.com/7brXKaukR4C
    double azi_half_offset =
        atan2(sin(half_angle_temp),
              cos(playbook_entry->elevation) * cos(half_angle_temp));
