...
double motor_speeds[kNumMotors] = {0};  // Maintain motor state locally.
...
void InitMotorControl(const double rotor_omegas[]) {
  for (int i = 0; i < kNumMotors; i++) {
    ...
    motor_speeds[i] = rotor_omegas[i];
    rate_limited_cmd[i] = rotor_omegas[i];
    cmd_filter_z[i] = rotor_omegas[i];
  }
}
  ...
  for (int32_t motor_number = 0; motor_number < kNumMotors; motor_number++) {
    ...
    // Apply Rate Limit
     ...
     omega_cmd = RateLimit(omega_cmd, -motor_params->omega_cmd_rate_limit,
                           ...
                           dt_motor_loop, &rate_limited_cmd[motor_number]);
    #endif

    // Apply command filter
    double a = -exp(dt_motor_loop * motor_params->speed_cmd_pole);
    double a_array[2] = {1.0, a};
    double b_array[2] = {1 + a, 0.0};
    double omega_cmd_filt = Filter(omega_cmd, 2, a_array, b_array, &cmd_filter_z[motor_number]);

    // Calculate reachable motor speeds
    double motor_bus_voltage = power_sys_params->v_source_0 / kNumMotors * 2.0;
    TorqueLimits torque_limit = CalcTorqueLimits(motor_bus_voltage, motor_speeds[motor_number],
                              &(motor_params->motor));

    // Apply control with limits and anti-windup
    double omega_error = omega_cmd_filt - motor_speeds[motor_number];
    double kp = motor_params->kp_rotor_vel_err;
    double torque = kp * omega_error + omega_cmd_int[motor_number];

    double ki = motor_params->ki_rotor_vel_err * dt_motor_loop;
    if (torque >= torque_limit.upper_limit) {
      ...
      omega_cmd_int[motor_number] += ki * omega_error;
    }
    ...
    motor_speeds[motor_number] += rotor_accel * dt_motor_loop;

    // Copy variables to output
    rotor_torques[motor_number] = torque;
    rotor_omegas[motor_number] = motor_speeds[motor_number];
  }
}
