...
subroutine KFAST_OS_ProcessOutputs()
   ...
   m%AllOuts(PtfmSurge) = m%FusO(1)
   m%AllOuts(PtfmSway ) = m%FusO(2)
   m%AllOuts(PtfmHeave) = m%FusO(3)
   val3 = R2D_D*EulerExtract(m%FusODCM)
   m%AllOuts(PtfmRoll ) = val3(1)
   m%AllOuts(PtfmPitch) = val3(2)
   m%AllOuts(PtfmYaw  ) = val3(3)
   val3 = matmul(m%FusODCM,m%FusOv)
   m%AllOuts(PtfmTVx  ) = val3(1)
   m%AllOuts(PtfmTVy  ) = val3(2)
   m%AllOuts(PtfmTVz  ) = val3(3)
   val3 = R2D_D*matmul(m%FusODCM,m%FusOomegas)
   m%AllOuts(PtfmRVx  ) = val3(1)
   m%AllOuts(PtfmRVy  ) = val3(2)
   m%AllOuts(PtfmRVz  ) = val3(3)
   val3 = matmul(m%FusODCM,m%FusOaccs)
   m%AllOuts(PtfmTAx  ) = val3(1)
   m%AllOuts(PtfmTAy  ) = val3(2)
   m%AllOuts(PtfmTAz  ) = val3(3)
   val3 = R2D_D*matmul(m%FusODCM,m%FusOalphas)
   m%AllOuts(PtfmRAx  ) = val3(1)
   m%AllOuts(PtfmRAy  ) = val3(2)
   m%AllOuts(PtfmRAz  ) = val3(3)
...
! -------------------------------------------------------------------------      
   ...
   if (printSum == 1) then  
      ...
      !call KFAST_OS_WriteSummary( SumFileUnit, p_OS, m_OS, HD_InitOut, MD_Mooring_InitOut, ErrStat2, ErrMsg2 )
      close(SumFileUnit)
...
subroutine Ass_Res_OffShore(t_c, isInitialTime_c, PtfmO_c, PtfmODCM_c, PtfmOv_c, PtfmOomegas_c, PtfmOacc_c, PtfmOalphas_c, &
   ...
   m_OS%HD%u(1)%Mesh%Orientation  (:,:,1) = reshape(PtfmODCM_c,(/3,3/))
