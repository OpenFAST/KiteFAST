...
subroutine Set_u( t, numTimes, numPylons, numFlaps, lastInd, Motions, HWindSpd, HWindDir, RefHt, PLexp, MeshMapData, u, p, z, errStat, errMsg )
   ...
   real(ReKi)        :: fusO(3)         ! Fuselage origin point in global coordinates
...
subroutine ReadMotions( UnIn, fileName, NumTimes, NumFlaps, NumPylons, Motions, errStat, errMsg, UnEc )
   ...
   integer(intKi)                               :: i,k,c,r, NumVals  ! counters
...
subroutine KAD_Dvr_ValidateInitData( dvrInitInp, errStat, errMsg )
   ...
   if ( ( dvrInitInp%HWindDir  < -PI/2.0_Reki ) .or.  ( dvrInitInp%HWindDir  > PI/2.0_Reki ) ) call SetErrStat( ErrID_Fatal, 'The reference wind direction must be between [-180,180]', errStat, errMsg, routineName )
   ...
   do i = 1, dvrInitInp%NumTimes
      ...
      if ( ( dvrInitInp%Motions%Roll(i)  < -PI/2.0_Reki ) .or.  ( dvrInitInp%Motions%Roll(i)  > PI/2.0_Reki ) ) then
      ...
      if ( ( dvrInitInp%Motions%Pitch(i)  < -PI/2.0_Reki ) .or.  ( dvrInitInp%Motions%Pitch(i)  > PI/2.0_Reki ) ) then
      ...
      if ( ( dvrInitInp%Motions%Yaw(i)  < -PI/2.0_Reki ) .or.  ( dvrInitInp%Motions%Yaw(i)  > PI/2.0_Reki ) ) then
      ...
      do j = 1, dvrInitInp%KAD_InitInp%NumPylons
         ...
         if ( ( dvrInitInp%Motions%SRtPitch(i,1,j)  < -180.0_ReKi ) .or.  ( dvrInitInp%Motions%SRtPitch(i,1,j)  > 180.0_ReKi ) ) then
         ...
         if ( ( dvrInitInp%Motions%SRtPitch(i,2,j)  < -180.0_ReKi ) .or.  ( dvrInitInp%Motions%SRtPitch(i,2,j)  > 180.0_ReKi ) ) then
         ...
         if ( ( dvrInitInp%Motions%PRtPitch(i,1,j)  < -180.0_ReKi ) .or.  ( dvrInitInp%Motions%PRtPitch(i,1,j)  > 180.0_ReKi ) ) then
         ...
         if ( ( dvrInitInp%Motions%PRtPitch(i,2,j)  < -180.0_ReKi ) .or.  ( dvrInitInp%Motions%PRtPitch(i,2,j)  > 180.0_ReKi ) ) then
...
subroutine KAD_Dvr_Simulate( dvrInitInp, errStat, errMsg )
   ...
   type(KAD_InputType)              :: KAD_u           ! Inputs at Time
