...
int32_t BoundedLeastSquares(const Mat *A, const Vec *b, const Vec *lower,
  ...
  // TODO(kennyjensen): This seems to be a good number for
  ...
  // TODO(kennyjensen): Justify the 1e-9 tolerance.
...
static bool IsOptimalAtConstraints(const Mat *A, const Vec *x, const Vec *b,
  ...
  assert(!err);
  // TODO(tobenkin): This statement suppresses warnings generated when compiling
...
int32_t ConstrainedLeastSquares(const Mat *A, const Vec *b, const Mat *C,
  ...
  while (num_iter < max_iter) {
    ...
    for (int32_t i = 0; i < C->nr; ++i) {
      if (!active[i]) {
        ...
        // active constraints.
        //
        // TODO(kennyjensen): What the numerical tolerance should be
  ...
  // TODO(kennyjensen): Justify the 1e-8 tolerance.  Note that we
