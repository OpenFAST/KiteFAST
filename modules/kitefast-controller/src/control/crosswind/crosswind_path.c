...
void CrosswindPathCalcBestHeading(const Vec3 *target_pos_cw,
  ...
  // in the upper half of the loop (when the kite has flown beyond
  // the 9 o'clock loop position).
  ...
  Vec3Scale(target_pos_cw, 1.0 / radius, &e_rad);
...
//
// TODO(kennyjensen): Revisit whether to account for this curvature
...
void CrosswindPathStep(FlightMode flight_mode, const Vec3 *path_center_g,
  ...
  *k_geom_curr_f = Lpf(Saturate(k_geom_curr, -0.1, 0.1), params->fc_k_geom_curr,
  ...
  CalcCosts(target_pos_cw, target_vel_cw, path_radius_cmd, path_type, params,
            costs);
  ...
  //
  // TODO(mabraham): Consider replacing the crosstrack controller.
  ...
  double crosstrack_radius_error_limited =
      Saturate(path_radius_cmd - current_pos_radius,
               -params->experimental_crosswind_max_radius_error,
               params->experimental_crosswind_max_radius_error);
