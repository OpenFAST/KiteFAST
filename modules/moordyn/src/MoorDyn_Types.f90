...
! =========  MD_InitInputType  =======
  TYPE, PUBLIC :: MD_InitInputType
    ...
    REAL(ReKi) , DIMENSION(1:3)  :: PtfmPos      !< initial position of platform [-]
    REAL(ReKi) , DIMENSION(1:3,1:3)  :: PtfmDCM      !< initial dcm of platform [-]
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  Int_Xferred = 1
      ...
      Re_Xferred   = Re_Xferred   + 1
      ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%PtfmPos))-1 ) = PACK(InData%PtfmPos,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%PtfmPos)
      ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%PtfmDCM))-1 ) = PACK(InData%PtfmDCM,.TRUE.)
 ...
 ! buffers to store meshes, if any
  ...
  Int_Xferred  = 1
      ...
      Re_Xferred   = Re_Xferred + 1
    i1_l = LBOUND(OutData%PtfmPos,1)
    i1_u = UBOUND(OutData%PtfmPos,1)
    ...
    END IF
    mask1 = .TRUE. 
      OutData%PtfmPos = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%PtfmPos))-1 ), mask1, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%PtfmPos)
    DEALLOCATE(mask1)
    i1_l = LBOUND(OutData%PtfmDCM,1)
    i1_u = UBOUND(OutData%PtfmDCM,1)
    i2_l = LBOUND(OutData%PtfmDCM,2)
    i2_u = UBOUND(OutData%PtfmDCM,2)
    ...
    END IF
    mask2 = .TRUE. 
      OutData%PtfmDCM = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%PtfmDCM))-1 ), mask2, 0.0_ReKi )
