...
! =========  MD_InitInputType  =======
  TYPE, PUBLIC :: MD_InitInputType
    ...
    INTEGER(IntKi)  :: NBodies      !< number of floating bodies with fairleads []
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: PtfmPos      !< one for each body: initial position of platform [-]
    REAL(ReKi) , DIMENSION(:,:,:), ALLOCATABLE  :: PtfmDCM      !< one for each body: initial DCM of platform [-]
...
! =========  MD_Connect  =======
  TYPE, PUBLIC :: MD_Connect
    ...
    INTEGER(IntKi)  :: BodyId      !< integer identifier of the body this connection is attached to (only for fairleads) [-]
...
! =========  MD_InitOutputType  =======
  TYPE, PUBLIC :: MD_InitOutputType
    ...
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: Anchs      !< Anchor type Connection positions [-]
...
! =========  MD_MiscVarType  =======
  TYPE, PUBLIC :: MD_MiscVarType
    ...
    INTEGER(IntKi) , DIMENSION(:,:), ALLOCATABLE  :: FairIdList      !< one for each body: array of size NFairs listing the ID of each fairlead (index of ConnectList) []
...
! =========  MD_ParameterType  =======
  TYPE, PUBLIC :: MD_ParameterType
    ...
    INTEGER(IntKi)  :: NBodies      !< number of floating bodies with fairleads []
    INTEGER(IntKi) , DIMENSION(:), ALLOCATABLE  :: NFairs      !< for each body: number of Fairlead Connections []
...
! =========  MD_InputType  =======
  TYPE, PUBLIC :: MD_InputType
    TYPE(MeshType) , DIMENSION(:), ALLOCATABLE  :: PtFairleadDisplacement      !< one for each body: mesh for position AND VELOCITY of each fairlead in X,Y,Z [[m, m/s]]
  END TYPE MD_InputType
! =======================
! =========  MD_OutputType  =======
  TYPE, PUBLIC :: MD_OutputType
    TYPE(MeshType) , DIMENSION(:), ALLOCATABLE  :: PtFairleadLoad      !< one for each body: point mesh for fairlead forces in X,Y,Z [[N]]
...
! 
   ...
   ErrMsg  = ""
    ...
    DstInitInputData%NBodies = SrcInitInputData%NBodies
IF (ALLOCATED(SrcInitInputData%PtfmPos)) THEN
  i1_l = LBOUND(SrcInitInputData%PtfmPos,1)
  i1_u = UBOUND(SrcInitInputData%PtfmPos,1)
  i2_l = LBOUND(SrcInitInputData%PtfmPos,2)
  i2_u = UBOUND(SrcInitInputData%PtfmPos,2)
  IF (.NOT. ALLOCATED(DstInitInputData%PtfmPos)) THEN 
    ALLOCATE(DstInitInputData%PtfmPos(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%PtfmPos.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitInputData%PtfmPos = SrcInitInputData%PtfmPos
ENDIF
IF (ALLOCATED(SrcInitInputData%PtfmDCM)) THEN
  i1_l = LBOUND(SrcInitInputData%PtfmDCM,1)
  i1_u = UBOUND(SrcInitInputData%PtfmDCM,1)
  i2_l = LBOUND(SrcInitInputData%PtfmDCM,2)
  i2_u = UBOUND(SrcInitInputData%PtfmDCM,2)
  i3_l = LBOUND(SrcInitInputData%PtfmDCM,3)
  i3_u = UBOUND(SrcInitInputData%PtfmDCM,3)
  IF (.NOT. ALLOCATED(DstInitInputData%PtfmDCM)) THEN 
    ALLOCATE(DstInitInputData%PtfmDCM(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitInputData%PtfmDCM.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitInputData%PtfmDCM = SrcInitInputData%PtfmDCM
ENDIF
...
! 
  ErrStat = ErrID_None
  ErrMsg  = ""
IF (ALLOCATED(InitInputData%PtfmPos)) THEN
  DEALLOCATE(InitInputData%PtfmPos)
ENDIF
IF (ALLOCATED(InitInputData%PtfmDCM)) THEN
  DEALLOCATE(InitInputData%PtfmDCM)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! PtfmDCM allocated yes/no
  IF ( ALLOCATED(InData%PtfmDCM) ) THEN
    Int_BufSz   = Int_BufSz   + 2*3  ! PtfmDCM upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%PtfmDCM)  ! PtfmDCM
  END IF
  ...
  Int_Xferred = 1
      ...
      Re_Xferred   = Re_Xferred   + 1
      IntKiBuf ( Int_Xferred:Int_Xferred+(1)-1 ) = InData%NBodies
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      IF (SIZE(InData%PtfmPos)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%PtfmPos))-1 ) = PACK(InData%PtfmPos,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%PtfmPos)
  END IF
  IF ( .NOT. ALLOCATED(InData%PtfmDCM) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%PtfmDCM,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%PtfmDCM,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%PtfmDCM,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%PtfmDCM,2)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%PtfmDCM,3)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%PtfmDCM,3)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%PtfmDCM)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%PtfmDCM))-1 ) = PACK(InData%PtfmDCM,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%PtfmDCM)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%PtfmPos)>0) OutData%PtfmPos = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%PtfmPos))-1 ), mask2, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%PtfmPos)
    DEALLOCATE(mask2)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! PtfmDCM not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%PtfmDCM)) DEALLOCATE(OutData%PtfmDCM)
    ALLOCATE(OutData%PtfmDCM(i1_l:i1_u,i2_l:i2_u,i3_l:i3_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%PtfmDCM.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ...
    END IF
    mask3 = .TRUE. 
      IF (SIZE(OutData%PtfmDCM)>0) OutData%PtfmDCM = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%PtfmDCM))-1 ), mask3, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%PtfmDCM)
    DEALLOCATE(mask3)
  END IF
...
! 
   ...
   ErrMsg  = ""
    ...
    DstConnectData%BodyId = SrcConnectData%BodyId
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  Int_Xferred = 1
      ...
      Int_Xferred   = Int_Xferred   + 1
      IntKiBuf ( Int_Xferred:Int_Xferred+(1)-1 ) = InData%BodyId
      Int_Xferred   = Int_Xferred   + 1
 ...
 ! buffers to store meshes, if any
  ...
  Int_Xferred  = 1
      ...
      Int_Xferred   = Int_Xferred + 1
      OutData%BodyId = IntKiBuf( Int_Xferred ) 
      Int_Xferred   = Int_Xferred + 1
...
ENDIF
    ...
    DstInitOutputData%NAnchs = SrcInitOutputData%NAnchs
IF (ALLOCATED(SrcInitOutputData%Anchs)) THEN
  i1_l = LBOUND(SrcInitOutputData%Anchs,1)
  i1_u = UBOUND(SrcInitOutputData%Anchs,1)
  i2_l = LBOUND(SrcInitOutputData%Anchs,2)
  i2_u = UBOUND(SrcInitOutputData%Anchs,2)
  IF (.NOT. ALLOCATED(DstInitOutputData%Anchs)) THEN 
    ALLOCATE(DstInitOutputData%Anchs(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstInitOutputData%Anchs.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstInitOutputData%Anchs = SrcInitOutputData%Anchs
ENDIF
...
IF (ALLOCATED(InitOutputData%Anchs)) THEN
  DEALLOCATE(InitOutputData%Anchs)
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  Int_BufSz   = Int_BufSz   + 1     ! Anchs allocated yes/no
  IF ( ALLOCATED(InData%Anchs) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! Anchs upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%Anchs)  ! Anchs
  END IF
  IF ( Re_BufSz  .GT. 0 ) THEN 
     ALLOCATE( ReKiBuf(  Re_BufSz  ), STAT=ErrStat2 )
     IF (ErrStat2 /= 0) THEN 
  ...
  END IF
      ...
      Int_Xferred   = Int_Xferred   + 1
  IF ( .NOT. ALLOCATED(InData%Anchs) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Anchs,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Anchs,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%Anchs,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%Anchs,2)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%Anchs)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%Anchs))-1 ) = PACK(InData%Anchs,.TRUE.)
      Re_Xferred   = Re_Xferred   + SIZE(InData%Anchs)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  END IF
      ...
      Int_Xferred   = Int_Xferred + 1
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! Anchs not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%Anchs)) DEALLOCATE(OutData%Anchs)
    ALLOCATE(OutData%Anchs(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%Anchs.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%Anchs)>0) OutData%Anchs = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%Anchs))-1 ), mask2, 0.0_ReKi )
      Re_Xferred   = Re_Xferred   + SIZE(OutData%Anchs)
    DEALLOCATE(mask2)
  END IF
...
ENDIF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%FairIdList)>0) OutData%FairIdList = UNPACK( IntKiBuf ( Int_Xferred:Int_Xferred+(SIZE(OutData%FairIdList))-1 ), mask2, 0_IntKi )
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      IF (SIZE(InData%NFairs)>0) IntKiBuf ( Int_Xferred:Int_Xferred+(SIZE(InData%NFairs))-1 ) = PACK(InData%NFairs,.TRUE.)
      Int_Xferred   = Int_Xferred   + SIZE(InData%NFairs)
  END IF
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%NFairs)>0) OutData%NFairs = UNPACK( IntKiBuf ( Int_Xferred:Int_Xferred+(SIZE(OutData%NFairs))-1 ), mask1, 0_IntKi )
      Int_Xferred   = Int_Xferred   + SIZE(OutData%NFairs)
    DEALLOCATE(mask1)
  END IF
...
IF (ALLOCATED(u_out%PtFairleadDisplacement) .AND. ALLOCATED(u1%PtFairleadDisplacement)) THEN
  DO i01 = LBOUND(u_out%PtFairleadDisplacement,1),UBOUND(u_out%PtFairleadDisplacement,1)
      CALL MeshExtrapInterp1(u1%PtFairleadDisplacement(i01), u2%PtFairleadDisplacement(i01), tin, u_out%PtFairleadDisplacement(i01), tin_out, ErrStat2, ErrMsg2 )
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
   ENDDO
...
IF (ALLOCATED(u_out%PtFairleadDisplacement) .AND. ALLOCATED(u1%PtFairleadDisplacement)) THEN
  DO i01 = LBOUND(u_out%PtFairleadDisplacement,1),UBOUND(u_out%PtFairleadDisplacement,1)
      CALL MeshExtrapInterp2(u1%PtFairleadDisplacement(i01), u2%PtFairleadDisplacement(i01), u3%PtFairleadDisplacement(i01), tin, u_out%PtFairleadDisplacement(i01), tin_out, ErrStat2, ErrMsg2 )
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
   ENDDO
END IF ! check if allocated
...
IF (ALLOCATED(y_out%PtFairleadLoad) .AND. ALLOCATED(y1%PtFairleadLoad)) THEN
  DO i01 = LBOUND(y_out%PtFairleadLoad,1),UBOUND(y_out%PtFairleadLoad,1)
      CALL MeshExtrapInterp1(y1%PtFairleadLoad(i01), y2%PtFairleadLoad(i01), tin, y_out%PtFairleadLoad(i01), tin_out, ErrStat2, ErrMsg2 )
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
   ENDDO
END IF ! check if allocated
...
IF (ALLOCATED(y_out%PtFairleadLoad) .AND. ALLOCATED(y1%PtFairleadLoad)) THEN
  DO i01 = LBOUND(y_out%PtFairleadLoad,1),UBOUND(y_out%PtFairleadLoad,1)
      CALL MeshExtrapInterp2(y1%PtFairleadLoad(i01), y2%PtFairleadLoad(i01), y3%PtFairleadLoad(i01), tin, y_out%PtFairleadLoad(i01), tin_out, ErrStat2, ErrMsg2 )
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
   ENDDO
END IF ! check if allocated
IF (ALLOCATED(y_out%WriteOutput) .AND. ALLOCATED(y1%WriteOutput)) THEN
