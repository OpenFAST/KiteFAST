...
! =========  KFAST_MiscVarType  =======
  TYPE, PUBLIC :: KFAST_MiscVarType
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FusOv      !<  [m/s]
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FusOomegas      !<  [rad/s]
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FusOaccs      !<  [m/s^2]
    ...
    REAL(ReKi) , DIMENSION(:), ALLOCATABLE  :: FusOalphas      !<  [rad/s^2]
    ...
    REAL(R8Ki) , DIMENSION(:,:), ALLOCATABLE  :: FusODCM      !<  [-]
...
ENDIF
IF (ALLOCATED(SrcMiscData%FusOv)) THEN
  i1_l = LBOUND(SrcMiscData%FusOv,1)
  i1_u = UBOUND(SrcMiscData%FusOv,1)
  IF (.NOT. ALLOCATED(DstMiscData%FusOv)) THEN 
    ALLOCATE(DstMiscData%FusOv(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%FusOv.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
IF (ALLOCATED(SrcMiscData%FusOomegas)) THEN
  i1_l = LBOUND(SrcMiscData%FusOomegas,1)
  i1_u = UBOUND(SrcMiscData%FusOomegas,1)
  IF (.NOT. ALLOCATED(DstMiscData%FusOomegas)) THEN 
    ALLOCATE(DstMiscData%FusOomegas(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%FusOomegas.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
IF (ALLOCATED(SrcMiscData%FusOaccs)) THEN
  i1_l = LBOUND(SrcMiscData%FusOaccs,1)
  i1_u = UBOUND(SrcMiscData%FusOaccs,1)
  IF (.NOT. ALLOCATED(DstMiscData%FusOaccs)) THEN 
    ALLOCATE(DstMiscData%FusOaccs(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%FusOaccs.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
IF (ALLOCATED(SrcMiscData%FusOalphas)) THEN
  i1_l = LBOUND(SrcMiscData%FusOalphas,1)
  i1_u = UBOUND(SrcMiscData%FusOalphas,1)
  IF (.NOT. ALLOCATED(DstMiscData%FusOalphas)) THEN 
    ALLOCATE(DstMiscData%FusOalphas(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%FusOalphas.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
IF (ALLOCATED(SrcMiscData%FusODCM)) THEN
  i1_l = LBOUND(SrcMiscData%FusODCM,1)
  i1_u = UBOUND(SrcMiscData%FusODCM,1)
  i2_l = LBOUND(SrcMiscData%FusODCM,2)
  i2_u = UBOUND(SrcMiscData%FusODCM,2)
  IF (.NOT. ALLOCATED(DstMiscData%FusODCM)) THEN 
    ALLOCATE(DstMiscData%FusODCM(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%FusODCM.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
IF (ALLOCATED(MiscData%FusOv)) THEN
  DEALLOCATE(MiscData%FusOv)
ENDIF
...
ENDIF
IF (ALLOCATED(MiscData%FusOomegas)) THEN
  DEALLOCATE(MiscData%FusOomegas)
ENDIF
...
ENDIF
IF (ALLOCATED(MiscData%FusOaccs)) THEN
  DEALLOCATE(MiscData%FusOaccs)
ENDIF
...
ENDIF
IF (ALLOCATED(MiscData%FusOalphas)) THEN
  DEALLOCATE(MiscData%FusOalphas)
ENDIF
...
ENDIF
IF (ALLOCATED(MiscData%FusODCM)) THEN
  DEALLOCATE(MiscData%FusODCM)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FusOv allocated yes/no
  IF ( ALLOCATED(InData%FusOv) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FusOv upper/lower bounds for each dimension
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FusOomegas allocated yes/no
  IF ( ALLOCATED(InData%FusOomegas) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FusOomegas upper/lower bounds for each dimension
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FusOaccs allocated yes/no
  IF ( ALLOCATED(InData%FusOaccs) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FusOaccs upper/lower bounds for each dimension
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FusOalphas allocated yes/no
  IF ( ALLOCATED(InData%FusOalphas) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! FusOalphas upper/lower bounds for each dimension
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! FusODCM allocated yes/no
  IF ( ALLOCATED(InData%FusODCM) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! FusODCM upper/lower bounds for each dimension
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%FusOv) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FusOv,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FusOv,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FusOv)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FusOv))-1 ) = PACK(InData%FusOv,.TRUE.)
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%FusOomegas) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FusOomegas,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FusOomegas,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FusOomegas)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FusOomegas))-1 ) = PACK(InData%FusOomegas,.TRUE.)
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%FusOaccs) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FusOaccs,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FusOaccs,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FusOaccs)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FusOaccs))-1 ) = PACK(InData%FusOaccs,.TRUE.)
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%FusOalphas) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FusOalphas,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FusOalphas,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FusOalphas)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%FusOalphas))-1 ) = PACK(InData%FusOalphas,.TRUE.)
  ...
  END IF
  IF ( .NOT. ALLOCATED(InData%FusODCM) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FusODCM,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FusODCM,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%FusODCM,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%FusODCM,2)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%FusODCM)>0) DbKiBuf ( Db_Xferred:Db_Xferred+(SIZE(InData%FusODCM))-1 ) = PACK(InData%FusODCM,.TRUE.)
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    DEALLOCATE(mask3)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FusOv not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FusOv)) DEALLOCATE(OutData%FusOv)
    ALLOCATE(OutData%FusOv(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FusOv.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FusOv)>0) OutData%FusOv = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FusOv))-1 ), mask1, 0.0_ReKi )
  ...
  ELSE
    ...
    DEALLOCATE(mask3)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FusOomegas not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FusOomegas)) DEALLOCATE(OutData%FusOomegas)
    ALLOCATE(OutData%FusOomegas(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FusOomegas.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FusOomegas)>0) OutData%FusOomegas = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FusOomegas))-1 ), mask1, 0.0_ReKi )
  ...
  ELSE
    ...
    DEALLOCATE(mask3)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FusOaccs not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FusOaccs)) DEALLOCATE(OutData%FusOaccs)
    ALLOCATE(OutData%FusOaccs(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FusOaccs.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FusOaccs)>0) OutData%FusOaccs = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FusOaccs))-1 ), mask1, 0.0_ReKi )
  ...
  ELSE
    ...
    DEALLOCATE(mask3)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FusOalphas not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FusOalphas)) DEALLOCATE(OutData%FusOalphas)
    ALLOCATE(OutData%FusOalphas(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FusOalphas.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%FusOalphas)>0) OutData%FusOalphas = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%FusOalphas))-1 ), mask1, 0.0_ReKi )
  ...
  ELSE
    ...
    DEALLOCATE(mask3)
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! FusODCM not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%FusODCM)) DEALLOCATE(OutData%FusODCM)
    ALLOCATE(OutData%FusODCM(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%FusODCM.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask2(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask2 = .TRUE. 
      IF (SIZE(OutData%FusODCM)>0) OutData%FusODCM = REAL( UNPACK(DbKiBuf( Db_Xferred:Db_Xferred+(SIZE(OutData%FusODCM))-1 ), mask2, 0.0_DbKi ), R8Ki)
