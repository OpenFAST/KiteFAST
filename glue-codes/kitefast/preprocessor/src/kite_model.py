...
class KiteModel(BaseModel):
    def __init__(self, simulation_dict, model_dict, simulation_type=1):
        super().__init__()
        ...
        if self.simulation_type == 1:
            ...
            for path in reference_point_paths:
                ...
                self._deep_put(simulation_dict, path, self._list_to_vec3(point))
        else:
            reference_point_paths = [
        ...
        if self.simulation_type > 1:
            self.platform = self.components[-1]
        ...
        # simulation setup
        self.main_mbd = MainMBD(self.simulation_type,
                                simulation_dict,
                                self.mip_node.id,
        ...
        self.main_set = MainSet(self.simulation_type, self.mip_reference_frame, self.components)
    ...
    def _build_components(self, model_dict):
        ...
        if self.simulation_type < 3:
            ...
            _components += [fuselage, starboard_wing, port_wing, vstab, starboard_hstab, port_hstab, starboard_pylons, port_pylons, starboard_rotors, port_rotors]
